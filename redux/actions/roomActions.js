...
export const getRooms =
  (req, currentPage = 1, limit = 3, location = "", guests, category) =>
  async (dispatch) => {
    try {
      const { origin } = absoluteUrl(req);

      let link = `${origin}/api/rooms?page=${currentPage}&limit=${limit}&location=${location}`;

      guests && link.concat(`&guestCapacity=${guests}`);
      category && link.concat(`&category=${category}`);
...
// NEW REVIEW
export const newReview = (reviewData, req) => async (dispatch) => {
  try {
...
export const checkReviewAvailability = (roomId, req) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${origin}/api/reviews/check_review_availability?roomId=${roomId}`
    );
    dispatch({
